...
sden = np.reshape(sden, (np.size(sden,axis=0)/12,12,np.size(sden,axis=1),np.size(sden,axis=2)))
sden = np.mean(sden,axis=1)

vwc  = data.variables['swvl1'][:] 
vwc = np.reshape(vwc, (np.size(vwc,axis=0)/12,12,np.size(vwc,axis=1),np.size(vwc,axis=2)))
